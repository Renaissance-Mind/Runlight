# Runlight Decoupling Implementation Plan
...
**Files:**
- Modify: `adapters/codex-hook/runlight-hook.sh`
- Modify: `adapters/claude-code-hook/runlight-hook.sh`
- Modify: `plugins/runlight-codex/skills/runlight/scripts/runlight-hook.sh`
- Modify: `plugins/runlight-codex/skills/runlight/SKILL.md`
...
In each shell hook, trim `RUNLIGHT_SERVER_URL`, remove trailing slashes before sending, and treat empty token as no Authorization header. Keep fail-open behavior.
...
```bash
bash -n adapters/codex-hook/runlight-hook.sh
bash -n adapters/claude-code-hook/runlight-hook.sh
bash -n plugins/runlight-codex/skills/runlight/scripts/runlight-hook.sh
...
**Files:**
- Modify: `server/runlight/config.py`
- Create: `server/tests/test_config.py` if not present, otherwise extend it
- Modify: `docs/superpowers/specs/2026-06-05-runlight-design.md`
...
```bash
cd server
RUNLIGHT_SERVER_HOST=127.0.0.1 RUNLIGHT_SERVER_PORT=8766 \
...
```bash
cd server
RUNLIGHT_TOKEN_MAP='tok-alice:alice' \
...
git commit -m "docs: document runlight deployment modes"
```
