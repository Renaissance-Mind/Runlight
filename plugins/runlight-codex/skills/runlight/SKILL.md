...
Runlight for Codex. Runlight is a passive observability system: it should
...
installer replaces older Runlight hook entries that point at previous plugin
cache paths.
...
`session_pin` to each Runlight event. `session_name` comes from the local
...
2. Confirm Runlight server is reachable at the configured `server_url`.
3. Start a fresh Codex thread and check the Runlight dashboard for a new
   `codex-hook` session.
