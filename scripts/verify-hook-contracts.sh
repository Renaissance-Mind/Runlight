...
LOCAL_ENRICH="$ROOT_DIR/src/local/enrich.js"
...
grep -q '/events/raw' "$LOCAL_HOOK"
grep -q 'buildCodexEvent' "$LOCAL_ENRICH"
grep -q 'buildClaudeEvent' "$LOCAL_ENRICH"
grep -q 'detectAutomation' "$LOCAL_ENRICH"
grep -q 'session_pin' "$LOCAL_ENRICH"
