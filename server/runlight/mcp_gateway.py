"""MCP Gateway — exposes Runlight tools via MCP protocol.
...
from runlight.protocol import EventEnvelope, MachineInfo, Severity, WorkspaceInfo
...
MCP_TOOLS = [
    ...
    {
        "name": "runlight_get_session_status",
...
def build_envelope_from_tool_call(tool_name: str, args: dict[str, Any]) -> EventEnvelope | None:
    ...
    if tool_name == "runlight_start_session":
    ...
    if tool_name == "runlight_record_event":
        return EventEnvelope(
            event_id=str(uuid.uuid4()),
            session_id=args["session_id"],
            agent_type=args["agent_type"],
    ...
    if tool_name == "runlight_heartbeat":
        return EventEnvelope(
            event_id=str(uuid.uuid4()),
            session_id=args["session_id"],
            agent_type=args["agent_type"],
    ...
    if tool_name == "runlight_finish_session":
        result = args["result"]
