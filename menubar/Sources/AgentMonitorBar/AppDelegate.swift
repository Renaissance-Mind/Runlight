...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if let button = statusItem.button {
            button.image = NSImage(systemSymbolName: "antenna.radiowaves.left.and.right", accessibilityDescription: "AgentMonitor")
    ...
    private func showContextMenu() {
        ...
        menu.addItem(withTitle: "Quit AgentMonitorBar", action: #selector(quitApp), keyEquivalent: "q")
    ...
    func updateStatusItemTitle() {
        ...
        if running > 0 || waiting > 0 {
            ...
            button.image = NSImage(
                ...
                accessibilityDescription: "AgentMonitor - \(total) active"
            )
        } else if let error = sessionStore.error {
            button.title = ""
            button.image = NSImage(
                systemSymbolName: "exclamationmark.triangle",
                accessibilityDescription: "AgentMonitor - Error: \(error)"
            )
        } else {
            button.title = ""
            button.image = NSImage(
