...
final class ServerConfigStore: ObservableObject {
    ...
    private enum Keys {
        static let serverUrl = "runlight.serverUrl"
        static let token = "runlight.token"
        static let refreshInterval = "runlight.refreshInterval"
    }
    ...
    init() {
        serverUrl = defaults.string(forKey: Keys.serverUrl)
            ?? defaults.string(forKey: LegacyKeys.serverUrl)
            ?? ServerConfig.defaultServerUrl
        token = defaults.string(forKey: Keys.token)
            ?? defaults.string(forKey: LegacyKeys.token)
            ?? ""
        let stored = defaults.double(forKey: Keys.refreshInterval)
        let legacyStored = defaults.double(forKey: LegacyKeys.refreshInterval)
        refreshInterval = stored > 0
            ? stored
            : (legacyStored > 0 ? legacyStored : ServerConfig.defaultRefreshInterval)
    }
